...
module.exports = {
  name: "lyrics",
  description: "Muestra las lyrics de la canción encontrada",
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] && !player) return client.sendTime(message.channel, "❌ | **Nada está en reproducción...**");
    if (!args[0]) SongTitle = player.queue.current.title;

    let lyrics = await lyricsFinder(SongTitle);
    if (!lyrics) return client.sendTime(message.channel, `**Ninguna letra se encontró para -** \`${SongTitle}\``);
    lyrics = lyrics.split("\n"); //spliting into lines
